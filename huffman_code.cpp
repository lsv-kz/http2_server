...
//======================================================================
static const unsigned int huffman_code_array[][3] = {
    {  0, 0x1ff8, 13},
    {  1, 0x7fffd8, 23},
    {  2, 0xfffffe2, 28},
    {  3, 0xfffffe3, 28},
...
//======================================================================
static int find_char(const unsigned int in, int size, int code_array_size, char *ch)
{
    int int_numbits = sizeof(int) * 8;
    for (int i = 0; i < code_array_size; ++i)
    {
        unsigned int d = huffman_code_array[i][1];
        int len = huffman_code_array[i][2];
        d = d << (int_numbits - len);
...
void huffman_decode(const char *s, int len, std::string& out)
{
    ...
    unsigned int buf;
    int code_array_size = sizeof(huffman_code_array)/(sizeof(int) * 3);
    ...
    for ( ; ; )
    {
        ...
        char ch;
        int n = find_char(fifo_buf, fifo_max_size - fifo_size, code_array_size, &ch);
        if (n > 0)
        {
            out += ch;
            fifo_buf = fifo_buf<<n;
...
int huffman_encode(const char *in, ByteArray& out)
{
    ...
    while (*in)
    {
        ...
        huff_buf = huffman_code_array[index][1];
        buf_len = huffman_code_array[index][2];
        if (index > 127)
